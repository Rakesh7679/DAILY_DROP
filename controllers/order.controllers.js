...
const formatPickupAddress = (shop) => {
    if (!shop) return ""
        ...
        if (user.role == "owner") {
            ...
            const filteredOrders = orders.map(order => {
                const shopOrder = order.shopOrders.find(o => {
                    const ownerVal = o.owner && o.owner._id ? o.owner._id : o.owner
                    return String(ownerVal) === String(req.userId)
                })
                return {
                    _id: order._id,
                    paymentMethod: order.paymentMethod,
...
export const updateOrderStatus = async (req, res) => {
    try {
                ...
                availableBoys.forEach(boy => {
                    ...
                    if (boySocketId) {
                        io.to(boySocketId).emit('newAssignment', {
                            ...
                            pickupAddress: formatPickupAddress(deliveryAssignment.shop),
        ...
        return res.status(200).json({
            ...
            pickupAddress: formatPickupAddress(updatedShopOrder?.shop)
...
export const getDeliveryBoyAssignment = async (req, res) => {
    try {
        ...
        const formated = assignments.map(a => ({
            ...
            pickupAddress: formatPickupAddress(a.shop),
...
export const rejectAssignment = async (req, res) => {
    try {
        ...
        if (assignment.status !== "brodcasted") {
            return res.status(400).json({ message: "cannot reject this assignment" })
        }

        // Mark as rejected/completed so it won't be shown again to this delivery boy
        assignment.status = 'completed'
        assignment.rejectedBy = req.userId
        ...
        return res.status(500).json({ message: `reject assignment error ${error}` })
    }
}
...
export const getCurrentOrder = async (req, res) => {
    try {
        const assignment = await DeliveryAssignment.findOne({
            ...
            .populate({
                ...
                populate: [
                    ...
                    { path: "shopOrders.shop", select: "name address city state" }
                ]
        ...
        if (assignment.assignedTo.location && assignment.assignedTo.location.coordinates && assignment.assignedTo.location.coordinates.length == 2) {
        ...
        return res.status(200).json({
            ...
            shopOrder,
            pickupAddress: formatPickupAddress(shopOrder?.shop),
